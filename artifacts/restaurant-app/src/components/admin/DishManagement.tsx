...
const evidenceLevels = [
  { value: "strong",   label: `Signature dish (strong)` },
  { value: "moderate", label: `Crowd favourite (moderate)` },
  { value: "weak",     label: `Worth a try (weak)` },
];
...
export function DishManagement({ restaurant, open, onOpenChange }: DishManagementProps) {
  ...
  const handleEdit = (dish: Dish) => { setEditingDish(dish); setIsFormOpen(true); };
  const handleCreate = () => { setEditingDish(null); setIsFormOpen(true); };

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl bg-white border-border max-h-[85vh] flex flex-col">
        <DialogHeader>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow><TableCell colSpan={5} className="text-center py-4 text-muted-foreground italic">No dishes yet</TableCell></TableRow>
              ) : (
                dishes.map((dish) => (
                  <TableRow key={dish.id} className="border-border/50">
                    ...
                    <TableCell className="text-sm text-muted-foreground">{formatEvidenceLabel(dish.evidenceLevel, "dish") || "—"}</TableCell>
...
function DishFormDialog({ open, onOpenChange, restaurantId, dish }: { open: boolean; onOpenChange: (open: boolean) => void; restaurantId: number; dish: Dish | null }) {
  ...
  const form = useForm<z.infer<typeof dishSchema>>({
    ...
    defaultValues: {
      name: "", category: "main", rawOrCooked: "", description: "",
      evidenceType: "", evidenceLevel: "", recommendationScore: "", dietTags: "", photoUrl: "",
    },
  });
  ...
  useState(() => {
    if (open) {
      if (dish) {
        form.reset({
          name: dish.name, category: dish.category, rawOrCooked: dish.rawOrCooked || "",
          description: dish.description || "", evidenceType: dish.evidenceType || "",
          evidenceLevel: dish.evidenceLevel || "", recommendationScore: dish.recommendationScore || "",
          dietTags: dish.dietTags || "", photoUrl: dish.photoUrl || "",
        });
      } else {
        form.reset({ name: "", category: "main", rawOrCooked: "", description: "", evidenceType: "", evidenceLevel: "", recommendationScore: "", dietTags: "", photoUrl: "" });
      }
    }
  });
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormField control={form.control} name="photoUrl" render={({ field }) => (
              <FormItem>
                <FormLabel>Dish photo URL</FormLabel>
                <FormControl><Input {...field} placeholder="https://example.com/dish.jpg" /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
